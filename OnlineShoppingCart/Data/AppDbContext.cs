...
namespace OnlineShoppingCart.Data
{
    public class AppDbContext : DbContext
    {
        ...
        public AppUserViewModel GetLoggedInUser()
        {
            ...
            //var userId = HttpContextAccessor.HttpContext.Session.GetString(Global.LoginGlobal);
            var token = HttpContextAccessor.HttpContext.Request.Cookies[Global.LoginCookieName]?.ToString();
            if (string.IsNullOrEmpty(token))
            {
                token = HttpContextAccessor.HttpContext.Request.Headers[Global.LoginCookieName].ToString();
            }
            if (!string.IsNullOrEmpty(token))
            {
                ...
                {
                    HttpContextAccessor.HttpContext.Response.Cookies.Delete(Global.LoginCookieName, new CookieOptions
                    {
                        IsEssential = true,
                    });
